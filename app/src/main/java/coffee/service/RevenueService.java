...
public class RevenueService {
    ...
    // 🟩 Chức năng 6: Hiển thị báo cáo doanh thu trực quan bằng console
    public void hienThiBaoCaoTrucQuan() {
        ...
        System.out.println("\n📊 BIỂU ĐỒ DOANH THU THEO NGÀY (Console Chart)");
        System.out.println("--------------------------------------------------");
        ...
        try (BufferedReader br = new BufferedReader(new FileReader(file))) {
            ...
            while ((line = br.readLine()) != null) {
                ...
                String ngay = parts[0].replace("Ngày:", "").trim();
                String doanhThuStr = parts[1].replaceAll("[^0-9]", "").trim();

                if (!doanhThuStr.isEmpty()) {
                    double doanhThu = Double.parseDouble(doanhThuStr);
                    doanhThuTheoNgay.put(ngay, doanhThu);
                    if (doanhThu > maxRevenue) maxRevenue = doanhThu;
                }
            }
        } catch (IOException e) {
            System.out.println("❌ Lỗi khi đọc file revenue.txt: " + e.getMessage());
            return;
        }

        if (doanhThuTheoNgay.isEmpty()) {
            System.out.println("❌ Không có dữ liệu doanh thu để hiển thị!");
            return;
        }

        // Vẽ biểu đồ cột (mỗi 10.000 VNĐ = 1 ký tự ▓)
        for (Map.Entry<String, Double> entry : doanhThuTheoNgay.entrySet()) {
            String ngay = entry.getKey();
            double doanhThu = entry.getValue();

            int length = (int) (doanhThu / 10000); // tỷ lệ thu nhỏ
            String bar = "▓".repeat(Math.max(1, Math.min(length, 100))); // giới hạn chiều dài tối đa

            System.out.printf("%s | %s %.0f VNĐ\n", ngay, bar, doanhThu);
        }

        System.out.println("--------------------------------------------------");
        System.out.println("💡 (Mỗi ký tự ▓ tương ứng ~10.000 VNĐ)");
    }
